...
with open("lineup.in", "r") as f:
    N = int(f.readline())
    for i in range(N):
        allConstraints.append(f.readline().split(" must be milked beside "))

for i in range(len(allConstraints)):
    allConstraints[i][0], allConstraints[i][1] = allConstraints[i][0].rstrip(), allConstraints[i][1].rstrip()

# for i in allConstraints:
    # print(i)
...
for constraint in allConstraints:
    # Get pair of cows, sort them
    cowOrder = []
    # Check if a cow has already been used
    if constraint[0] not in unusedCows and constraint[1] not in unusedCows:
        break
    elif constraint[0] not in unusedCows:
        # Check if either end of an existing group contains this cow
        for groupIdx in range(len(newGroups)):
            if newGroups[groupIdx][0] == constraint[0]:
                newGroups[groupIdx].insert(0, constraint[1])
                unusedCows[unusedCows.index(constraint[1])] = None
                break
            elif newGroups[groupIdx][len(newGroups[groupIdx])-1] == constraint[0]:
                newGroups[groupIdx].insert(len(newGroups[groupIdx])-1, constraint[1])
                unusedCows[unusedCows.index(constraint[1])] = None
                break
    elif constraint[1] not in unusedCows:
        # Check if either end of an existing group contains this cow
        for groupIdx in range(len(newGroups)):
            if newGroups[groupIdx][0] == constraint[1]:
                newGroups[groupIdx].insert(0, constraint[0])
                unusedCows[unusedCows.index(constraint[0])] = None
                break
            elif newGroups[groupIdx][len(newGroups[groupIdx]) - 1] == constraint[1]:
                newGroups[groupIdx].insert(len(newGroups[groupIdx]) - 1, constraint[0])
                unusedCows[unusedCows.index(constraint[0])] = None
                break
    else: # Both cows are unused
        cowAIndex, cowBIndex = unusedCows.index(constraint[0]), unusedCows.index(constraint[1])
        cowOrder = [constraint[0], constraint[1]]
        if constraint[0] == constraint[1]:
            del cowOrder[1]
        cowOrder.sort()
        # Mark used cows as nil
        unusedCows[cowAIndex], unusedCows[cowBIndex] = None, None

        # Add new pair to group
        for groupIdx in range(len(newGroups)):
            if cowOrder[0] < newGroups[groupIdx][0]:
                newGroups.insert(groupIdx, cowOrder)
                break
        else:
            newGroups.append(cowOrder)
    # print(newGroups)
...
for cow in unusedCows:
    for groupIdx in range(len(newGroups)):
        if cow < newGroups[groupIdx][0]:
            newGroups.insert(groupIdx, [cow])
            break
    else:
        newGroups.append([cow])
...
#             didIntersection = True
#             t1 = [i for i in constraints[i] if i not in intersection]
#             t2 = [i for i in constraints[i+1] if i not in intersection]
#             # t1.reverse()
#             # t2.reverse()
#             c = []
#             # print("t1", t1, "t2", t2)
...
#     if didIntersection:
#         constraints = tempConstraints.copy()
